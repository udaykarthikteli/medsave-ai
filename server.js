...
let GEMINI_MODEL = (process.env.GEMINI_MODEL || 'gemini-flash-latest').trim().replace(/^["']|["']$/g, '');

// gemini-2.5-flash (and the older 1.5/2.0 line) were retired by Google for
// new API keys — if this value is still lying around anywhere (an old
// Render env var, a stale .env, etc.) force the safe current default
// instead of letting every request fail with a 404.
if (/^gemini-(1\.5|2\.0|2\.5)-/i.test(GEMINI_MODEL)) {
  console.warn(`\n  ⚠️  GEMINI_MODEL="${GEMINI_MODEL}" is a retired model — overriding to "gemini-flash-latest". Remove/update the GEMINI_MODEL env var to stop seeing this.\n`);
...
// trust that header and throws on every request.
